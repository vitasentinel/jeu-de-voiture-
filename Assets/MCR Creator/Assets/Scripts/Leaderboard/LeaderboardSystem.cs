...
public class LeaderboardSystem : MonoBehaviour {
	...
	void Start () {																	// --> Initialization
		...
		if(TmpEvent){
			...
			Debug.Log("Info : An EventSystem is needed");}
		...
		if(txt_PlayerName){
			if (PlayerPrefs.HasKey ("LastTimeTrialPlayer"))
				txt_PlayerName.text = PlayerPrefs.GetString("LastTimeTrialPlayer");	
			else
				txt_PlayerName.text = "-";										// 
		}
	...
	public void InitPlayerScore(){
		if (txt_Leaderboard_Score) {
			txt_Leaderboard_Score.text = F_Timer(PlayerPrefs.GetInt ("CurrentScore")*.001f);
		}
	}
	...
	public void Validate () {														// --> UI Button is pressed. Validate name and score. Save them on a PlayerPrefs
		...
		PlayerPrefs.SetString ("LastTimeTrialPlayer", txt_PlayerName.text);					// Save the player name for the next time the board appear on screen
	}
	...
	public void UpdateLeaderboard(){												// --> Update the leaderboard 
		...
		for(int i  = 0; i < playersScores.Count; i++){									// Display all the score on the leaderboard panel
			if(i < MaxScoreDisplay){
				...
				if(txt_OnLeaderboard_Score)txt_OnLeaderboard_Score.text += s_color + F_Timer(int.Parse(playersScores[i].score) *.001f) + "\n" + "</color>";								// Display score on screen
